...
from features_relations import structural_number_tokens_source, relationship, structural_difference_number_tokens, \
    structural_sentence_distance, structural_target_before_source, lexical_number_common_terms, \
    structural_number_punctuation_marks_source, structural_difference_number_punctuation_marks, \
    lexical_source_contains_modal_verb, indicators_source_contains_discourse_markers
...
def predict(elements):
    ...
    for e in elements:
        if e['label'] == "claim":
            ...
            number_of_claims += 1
            supported = False
            for s in support_relations:
                if s['srcElem'] == e['id'] or s['trgElem'] == e['id']:
                    supported = True
                    break
            if not supported:
                number_unsupported_claims += 1

    return support_relations, number_unsupported_claims, number_of_claims
...
def encode_features(elements):
    ...
    col_names = ['number_tokens_source', 'number_tokens_target', 'difference_number_tokens', 'sentence_distance',
                 'number_of_common_terms', 'number_of_punctuation_marks_source', 'number_of_punctuation_marks_target',
...
def generate_features(pair1, pair2):
    ...
    features["difference_number_tokens"] = structural_difference_number_tokens.get_difference_number_tokens(pair1,
                                                                                                            pair2)  # int
    ...
    features[
        "number_of_punctuation_marks_source"] = structural_number_punctuation_marks_source.get_number_of_punctuation_marks(
        pair1['text'])  # int
    features[
        "number_of_punctuation_marks_target"] = structural_number_punctuation_marks_source.get_number_of_punctuation_marks(
        pair2['text'])  # int
    features[
        "difference_number_punctuation_marks"] = structural_difference_number_punctuation_marks.get_difference_number_of_punctuation_marks(
        pair1, pair2)  # int
    features["contains_modal_verb_source"] = lexical_source_contains_modal_verb.contains_modal_verb(
        pair1['text'])  # bool
    features["contains_modal_verb_target"] = lexical_source_contains_modal_verb.contains_modal_verb(
        pair2['text'])  # bool
    features[
        "contains_argumentative_discourse_markers_source"] = indicators_source_contains_discourse_markers.contains_argumentative_markers(
        pair1['text'])  # bool
    features[
        "contains_argumentative_discourse_markers_target"] = indicators_source_contains_discourse_markers.contains_argumentative_markers(
        pair2['text'])  # bool
    return features
